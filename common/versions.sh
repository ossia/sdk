...
# its find_package(SDL2) still speak the SDL2 API, and score's deployment builds
# hard-assert the `sdl` feature. Both SDLs are therefore installed side by side
# ($INSTALL_PREFIX/SDL2 and $INSTALL_PREFIX/SDL3) so an SDK tag stays consumable
# by today's score while the port happens in libossia. Drop SDL_VERSION and every
# sdl.sh once libossia is on SDL3.
